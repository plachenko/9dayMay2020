export default class AlignGrid {
    constructor(config) {
        ...
        if (!config.width) {
            config.width = config.scene.cameras.main.width;
        }
        if (!config.height) {
            config.height = config.scene.cameras.main.height;
        }
    ...
    placeBetween(idx1, idx2) {
        const pos1 = this.getIndexPos(idx1).dim;
        const pos2 = this.getIndexPos(idx2).dim;
        const obj = {};
        obj.x = pos1.x1;
        obj.y = pos1.y1;
        obj.w = pos2.x2 - pos1.x1;
        obj.h = pos2.y2 - pos1.y1;
